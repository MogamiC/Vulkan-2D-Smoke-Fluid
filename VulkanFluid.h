...
public:
	VulkanTest() {
		CreateGlfwWindow(1280, 960, "vulkan");
	...
	void Run() {
		...
		while (!glfwWindowShouldClose(mWindowPtr)) {
			...
			ProjectionInit();
			RkRk();
			while (((ProjectionUniform*)mProjectionUniformBufferMapped)->mRkRk < 0.000001) {
				PkAPk();
				Xk();
				Rk();
				RkRkIn();
				...
				if (((ProjectionUniform*)mProjectionUniformBufferMapped)->mRkRk < 0.000001) {
					break;
				}
			}
			Final();
...
private:
	...
	void CreateShaderStorageBuffers() {
		...
		sConstGrid.mVelocityV = 0.3;
		sConstGrid.mConcentration = 1;
		sConstGrid.mTemperature = 400;

		SmokeGridCell2D sEnvirGrid;
		sEnvirGrid.mCellType = 1;
		sEnvirGrid.mVelocityU = 0.1;
		sEnvirGrid.mVelocityV = 0.1;

		for (uint32_t i = 0; i < 800 * 600; ++i) {
			sArrayGridCell2D[i] = sEnvirGrid;
		}
		int a = 800 * 5;
		int b = 800 * 300;
